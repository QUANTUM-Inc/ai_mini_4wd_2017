...
static void _capture_done_cb(uint8_t status);
...
typedef union Lsm6ds3hImuData_t
{
	uint8_t bytes[12];
	AiMini4WdSensorsImuData imudata;
} Lsm6ds3hImuData;
static Lsm6ds3hImuData sRxBuf;
static uint8_t sTxbuf[1];


/*---------------------------------------------------------------------------*/
/*---------------------------------------------------------------------------*/
uint8_t lsm6ds3h_probe(void)
{
	uint8_t txbuf[2];
	uint8_t rxbuf[12];
	uint8_t ret = 0;
	...
	ret = i2c_txRxBytes(I2C_ON_PORTE, LSM6DS3H_ADR, txbuf, 1, rxbuf, 1);
	if (ret != 0) {
		return ret;
	}
	else if (rxbuf[0] != 0x69) {
		return AI_ERROR_UNEXPECTED_DEVICE;
	}
	...
	txbuf[1] = ctrl3_c;
	ret = i2c_txRxBytes(I2C_ON_PORTE, LSM6DS3H_ADR, txbuf, 2, NULL, 0);
	if (ret != 0) {
		return ret;
	}
	...
	// Setup Ctrl1 xl
	uint8_t ctrl1 = (0x07 << ODR_XL0_BP) | (0x01 << FS_XL0_BP) | (0x00 << BW_XL0_BP);
	...
	// Setup Ctrl2 g
	uint8_t ctrl2 = (0x07 << ODR_G0_BP) | (0x03 << FS_G0_BP);
	...
	txbuf[0] = LSM6DS3H_OUTX_L_G;
	ret = i2c_txRxBytes(I2C_ON_PORTE, LSM6DS3H_ADR, txbuf, 1, rxbuf, 12);
	if (ret != 0) {
		return ret;
	}
...
static void _checkDataReadyAndRead(void)
{
	...
	if ((sAccelReady != 0) && (sGyroReady != 0)) {
		...
		ret = -1;
		while ((ret != 0) && (ret != I2C_IS_IN_USED)) {
			ret = i2cSchedulerPushTask(LSM6DS3H_ADR, sTxbuf, 1, sRxBuf.bytes, 12, _capture_done_cb);
		}
	}
...
static void _capture_done_cb(uint8_t status)
{
	...
	aiMini4WdSensorsUpdate(&sRxBuf.imudata);
