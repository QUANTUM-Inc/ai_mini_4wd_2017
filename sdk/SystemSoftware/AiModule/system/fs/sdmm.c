...
#include "system/fs/diskio.h"		/* Common include file for FatFs and disk I/O layer */
...
{
	do {
		uint8_t ret = spi_rx(SPI_ON_PORTC);
...
)
{
	...
	for (n = 10; n; n--) (void)spi_txrx_pio(SPI_ON_PORTC, 0xff);	/* Apply 80 dummy clocks and the card gets ready to receive command */

	//J この後は、SCLKを叩くときは必ずSSがLoになるはず
	uint8_t ret =initialize_spi_master(SPI_ON_PORTC, SPI_TYPE_3WIRE, SPI_TRANSFER_MODE_0, SPI_DATA_ORDER_MSB_FIRST, SPI_CLK_DIV_2);
	if (ret != SPI_OK) {
		return (DSTATUS)ret;
	}
